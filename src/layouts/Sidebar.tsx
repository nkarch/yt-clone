...
function LargeSidebarSection({
    ...
    return (
        <div>
            {title && <div className='ml-1 mt-2 text-lg mb-1'>{title}</div>}
            {visibleChildren}
            {showExpandButton && (
                <Button
                    variant='ghost'
...
function LargeSidebarItem({ IconOrImgUrl, title, url, isActive = false }: LargeSidebarProps) {
    return (
        <a
            href={url}
            className={twMerge(
        ...
        >
            ...
            <div className='whitespace-nowrap overflow-hidden text-ellipses'>{title}</div>
        </a>
    );
}
